...
public class BookController {
    ...
    public ModelAndView queryBookDo(HttpServletRequest request,String searchWord){
        ...
        if (exist){
            ...
            return new ModelAndView("admin_books","error","Không có sách phù hợp");
        }
    }
    ...
    public String readerQueryBookDo(HttpServletRequest request,String searchWord,RedirectAttributes redirectAttributes){
        ...
        if (exist){
            ...
            redirectAttributes.addFlashAttribute("error", "Không có sách phù hợp!");
            return "redirect:/reader_querybook.html";
        }
    ...
    public String deleteBook(HttpServletRequest request,RedirectAttributes redirectAttributes){
        ...
        if (res==1){
            redirectAttributes.addFlashAttribute("succ", "Đã xóa sách thành công!");
            return "redirect:/allbooks.html";
        }else {
            redirectAttributes.addFlashAttribute("error", "Xóa sách không thành công!");
            return "redirect:/allbooks.html";
        }
    }
    ...
    public String addBookDo(BookAddCommand bookAddCommand,RedirectAttributes redirectAttributes){
        ...
        ArrayList<Book> books=bookService.getAllBooks();
        if (succ){
            redirectAttributes.addFlashAttribute("succ", "Đã thêm sách thành công!");
            return "redirect:/allbooks.html";
        }
        else {
            redirectAttributes.addFlashAttribute("error", "Không thể thêm sách!");
            return "redirect:/allbooks.html";
        }
    }
    ...
    public String bookEditDo(HttpServletRequest request,BookAddCommand bookAddCommand,RedirectAttributes redirectAttributes){
        ...
        if (succ){
            redirectAttributes.addFlashAttribute("succ", "Sửa sách thành công!");
            return "redirect:/allbooks.html";
        }
        else {
            redirectAttributes.addFlashAttribute("error", "Sửa đổi sách không thành công!");
            return "redirect:/allbooks.html";
        }
    }
